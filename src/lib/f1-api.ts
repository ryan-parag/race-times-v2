...
export async function fetchMeetings(year: number): Promise<Meeting[]> {
  try {
    const res = await fetchWithTimeout(`${OPENF1_BASE}/meetings?year=${year}`, {
    ...
    throw new Error(`Failed to fetch meetings: ${message}`);
  }
}
...
export async function fetchDrivers(meetingKey: number): Promise<Driver[]> {
  try {
    const res = await fetchWithTimeout(`${OPENF1_BASE}/drivers?meeting_key=${meetingKey}`, {
      next: { revalidate: 3600 },
    });
    if (!res.ok) return [];
